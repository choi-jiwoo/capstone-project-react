  ...
  const showResults = () =>
    data.map((item) => (
      <div key={item.id} className='m-4'>
        <div className='flex flex-col border-solid border-transparent shadow-md border- border-gray-500 rounded-lg py-4'>
          ...
          <button
            key={item.id}
            className='place-self-center border-solid border-2 border-green-400 text-green-400 hover:text-green-700 hover:border-green-700 rounded-lg py-2 px-3 text-sm'
            onClick={() => {
